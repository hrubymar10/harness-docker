...
for harness in claude codex pi vibe; do docker exec "$container" "$harness" --version >/dev/null; docker exec "$container" test -e "/this-is-$harness-docker-env"; done
docker exec "$container" test -L /usr/local/bin/claude-notifier
